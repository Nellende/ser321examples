            ...
            if (request == null) {
                ...
                if (request.length() == 0) {
                    ...
                    try {
                        ...
                        builder.append("Content-Type: text/html; charset=utf-8\n");
                        builder.append("\n");
                        builder.append("Error from Server");
                    } catch (StringIndexOutOfBoundsException e) {
                        builder.append("HTTP/1.1 428 Query Required\n");
                        builder.append("Content-Type: text/html; charset=utf-8\n");
                        builder.append("\n");
                        builder.append("Query was not given upon request");
                    }
                    ...
                    //gives you a random number between to set of given ints
                    //declare with localhost:port/randomNum?min=1&max=5
                    Map<String, String> query_pairs = new LinkedHashMap<String, String>();
                    try {
                        query_pairs = splitQuery(request.replace("randomNum?", ""));
                        try {
                            // extract required fields from parameters
                            ...
                            Integer result = null;
                            try {
                                if (min == 0) {// min = 0, max 5, gives numbers between 0-5
                                    result = (int) (Math.round(Math.random() * max));
                                } else if (min < 0) { // min = -1, max 5, gives number between -1-5
                                    result = (int) (Math.round(Math.random() * (max + 1)) + min);
                                } else { // base min = 1, max 6, 1-6 random number
                                    result = (int) (Math.round(Math.random() * (max - 1)) + min);
                                }
                            } catch (Exception e) {
                ...
                } else if (request.contains("fraction?")) {
                    //gives you a fraction set up from two numbers
                    //declare with localhost:port/fraction?num=1&denom=5
                    Map<String, String> query_pairs = new LinkedHashMap<String, String>();
                    try {
                        query_pairs = splitQuery(request.replace("fraction?", ""));
                        try {
                            // extract required fields from parameters
                            Integer num = Integer.parseInt(query_pairs.get("num"));
                            Integer denom = Integer.parseInt(query_pairs.get("denom"));
                            // do math
